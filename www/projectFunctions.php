...
function studentInfo() {
	...
	if(!empty($_GET["firstname"])) {
		...
		if(!preg_match("/^[A-Za-z0-9 .']*$/", $returnValue[1])) {
	...
	if(!empty($_GET["lastname"])) {
		...
		if(!preg_match("/^[A-Za-z0-9 .']*$/", $returnValue[2])) {
	...
	if(!empty($_GET["email"])) {
		...
		if(!filter_var($returnValue[3], FILTER_VALIDATE_EMAIL)) {
	...
	if(!empty($_GET["course"])) {
		...
		if(!preg_match("/^[A-Za-z]{3,4}[0-9]{3}[A-Za-z0-9]{0,1}$/", $returnValue[6])) {
